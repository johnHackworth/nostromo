...
( function() {
    ...
    shipView.prototype = {
        ...
        checkSurvivors: function() {
            var survivors = false;
            this.stage.playerBoat.people.forEach(function(person) {
                if (!person.xeno && person.model.health > 0) {
                    survivors = true;
                }
            })

            if (!survivors || this.stage.playerBoat.xenomorph.model.health <= 0) {
                setTimeout(this.finishGame.bind(this), 2000);
            }
        },
        finishGame: function() {
            if (!this.finished) {
                this.setTargetScale(0.01, 4);
                var text = 'You are dead.';
                if (this.stage.playerBoat.xenomorph.model.health > 0) {
                    text = 'You are alone now';
                }
                this.finished = true;
                this.finishText = this.stage.addTextToContainer(this.container,
                    text, {
                        x: 600,
                        y: 300,
                        centered: true,
        ...
        startGame: function() {
            var text = 'Move with arrows, attack with [space] or [enter]. Hide with left [ctrl]';
            this.startText = this.stage.addTextToContainer(this.zoomableContainer,
                text, {
                    x: 600,
                    y: 650,
                    centered: true,
            ...
            this.title = this.stage.addTextToContainer(this.zoomableContainer,
                'NOSTROMO', {
                    x: 600,
                    y: -300,
                    centered: true,
