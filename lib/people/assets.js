...
window.boot.assets.person = {
    ...
    initSoldierTextures: function() {
        this.soldierSpriteSheet = PIXI.Texture.fromImage('assets/people/soldier.sheet.png');

        this.textures.soldier = {};
        var i = 0;
        var h = 100;
        var w = 70 * 100 / 100;
        this.textures.soldier.standing = [];
        for (i = 0; i < 8; i++) {
            this.textures.soldier.standing.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 0, i));
        }
        this.textures.soldier.working = [];
        for (i = 1; i < 8; i++) {
            this.textures.soldier.working.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 0, i));
        }
        this.textures.soldier.running = [];
        for (i = 0; i < 9; i++) {
            this.textures.soldier.running.push(this.getTexture(this.soldierSpriteSheet, w, h, h, i));
        }
        this.textures.soldier.runningLeft = [];
        for (i = 0; i < 9; i++) {
            this.textures.soldier.runningLeft.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 2, i));
        }
        this.textures.soldier.climbing = [];
        for (i = 0; i < 4; i++) {
            this.textures.soldier.climbing.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 3, i));
        }
        this.textures.soldier.fixing = [];
        for (i = 0; i < 4; i++) {
            this.textures.soldier.fixing.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 4, i));
        }
        this.textures.soldier.fixing2 = [];
        for (i = 0; i < 6; i++) {
            this.textures.soldier.fixing2.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 5, i));
        }
        this.textures.soldier.standingColor = [];
        for (i = 0; i < 8; i++) {
            this.textures.soldier.standingColor.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 6, i));
        }
        this.textures.soldier.sideWork = [];
        for (i = 0; i < 1; i++) {
            this.textures.soldier.sideWork.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 7, i));
        }
        this.textures.soldier.death = [];
        for (i = 0; i < 6; i++) {
            this.textures.soldier.death.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 8, i));
        }

        this.textures.soldier.attack = [];
        for (i = 0; i < 2; i++) {
            this.textures.soldier.attack.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 9, i));
        }
        this.textures.soldier.attackLeft = [];
        for (i = 0; i < 2; i++) {
            this.textures.soldier.attackLeft.push(this.getTexture(this.soldierSpriteSheet, w, h, h * 10, i));
        }
    },
